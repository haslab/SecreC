#OPTIONS_SECREC --implicitcoercions=extendedc --backtrack=fullb --matching=unorderedm --promote=lpromote
...
//@ inline;
{
   ...
   //pd_shared3p uint[[2]] F_cache = reshape(classify({} :: public uint[[1]]) :: pd_shared3p uint[[1]],0,shape(db)[0]);
   f.items = F;
   f.cache = F_cache;
   return f;
}
