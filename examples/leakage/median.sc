...
int median (private int a1, private int a2, private int b1, private int b2) {
    ...
    if (declassify(a1 <= b1)) {a = a2; b = b1; } else { a = a1; b = b2; }

    private int res;
    if (declassify(a <= b)) { res = a; } else { res = b; }
    
    return declassify(res);
}

// the median by itself does not leak anything
int median2 (private int a1, private int a2, private int b1, private int b2) {
    private int a;
    private int b;
    private bool x = a1 <= b1;
