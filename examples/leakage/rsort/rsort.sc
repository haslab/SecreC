//#OPTIONS_SECREC --implicitcoercions=defaultsc --backtrack=tryb --matching=gorderedm --promote=nop --verify=bothv --entrypoints="bitwise_rsort" --nodafnymodules
...
private bool[[2]] bitwise_rsort (private bool[[2]] bits)
{
    ...
    //@ invariant 0 <= m && m <= k;
    {
        ...
        //@ invariant 0 <= i && i <= n;
        //@ invariant c0 + c1 == i;
        //@ free invariant forall uint j ; (0 <= j && j < i) ==> 0 <= ord[j] && ord[j] < n;
        //@ free invariant Distinct(ord);
        {
            c0 = c0 + classify(1) - bitColMod2n[i];
            c1 = c1 + bitColMod2n[i];
            ord[i] = (classify(1)-bitColMod2n[i]) * c0 + bitColMod2n[i] * (n0 + c1);
        }
