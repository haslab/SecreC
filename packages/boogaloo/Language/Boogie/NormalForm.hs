...
negationNF boolExpr = case node boolExpr of
  UnaryExpression Not e -> case node e of
    ...
    Quantified Forall tv vars trggs' e' -> attachPos (position e) $ Quantified Exists tv vars (applyToQTrigAtts (negationNF . enot) trggs') (negationNF (enot e'))
    Quantified Exists tv vars trggs' e' -> attachPos (position e) $ Quantified Forall tv vars (applyToQTrigAtts (negationNF . enot) trggs') (negationNF (enot e'))
    _ -> boolExpr
  ...
  Quantified qop tv vars trggs e -> attachPos (position boolExpr) $ Quantified qop tv vars (applyToQTrigAtts negationNF trggs) (negationNF e)
  _ -> boolExpr
...
prenexNF boolExpr = glue $ rawPrenex boolExpr
  where
    ...
    rawPrenex boolExpr = case node boolExpr of
      ...
      Quantified qop tv vars trggs e -> attachPos (position boolExpr) $ Quantified qop tv vars (applyToQTrigAtts rawPrenex trggs) (rawPrenex e)
      _ -> boolExpr
    merge r1 r2 op e1 e2 = attachPos (position e1) (merge' r1 r2 op e1 e2)
    merge' r1 r2 op (Pos _ be1@(Quantified _ _ _ _ _)) e2 = let Quantified qop tv' vars' trggs' e' = renameBound r1 be1
      in Quantified qop tv' vars' (applyToQTrigAtts (merge r1 r2 op e') trggs') (merge r1 r2 op e' e2)
    merge' r1 r2 op e1 (Pos _ be2@(Quantified _ _ _ _ _)) = let Quantified qop tv' vars' trggs' e' = renameBound r2 be2
      in Quantified qop tv' vars' (applyToQTrigAtts (merge r1 r2 op e1) trggs') (merge r1 r2 op e1 e')
    ...
    renameBound r (Quantified qop tv vars trggs e) = Quantified qop (map r tv) (map (renameVar r tv) vars) (applyToQTrigAtts (exprSubst (varBinding r (map fst vars))) trggs) (exprSubst (varBinding r (map fst vars)) e)
    ...
    glue' boolExpr = case boolExpr of
      Quantified qop tv vars trggs e -> case node e of
        Quantified qop' tv' vars' trggs' e' | qop == qop' -> glue' (Quantified qop (tv ++ tv') (vars ++ vars') (trggs ++ trggs') e')
                                     | otherwise -> Quantified qop tv vars trggs (glue e)
        _ -> boolExpr
      _ -> boolExpr
