...
getExemptions :: VCGen -> Program -> IO (Set Id,Type -> Bool)
getExemptions NoVCGen p = return (defaultExemptions,const False)
getExemptions Dafny p = do
    ...
    let exs = Set.unions [pdefs,defaultExemptions,Set.fromList ["$_reverifyPost","$_Frame"]]
...
dafnyIdModule (firstRE "[$.#]_[0-9]+_[a-zA-Z]*\\." -> Just m) = Just $ (!!1) $ splitOn "_" $ init $ tail $ tail m
dafnyIdModule (firstRE "[$.#][a-zA-Z]*\\." -> Just m) = Just $ init $ tail m
dafnyIdModule (firstRE "^_[0-9]+_[a-zA-Z]*\\." -> Just m) = Just $ (!!1) $ splitOn "_" $ init $ tail m
dafnyIdModule (firstRE "^[a-zA-Z]*\\." -> Just m) = Just $ init m
