...
--
module Data.HashTable.Weak.Class
  ( HashTable(..)
  , fromList
...
class HashTable h where
    ...
    new      :: (Eq k,Hashable k) => ST s (h s k v)

    -- | Creates a new hash table sized to hold @n@ elements. /O(n)/.
    newSized :: (Eq k,Hashable k) => Int -> ST s (h s k v)

    newSizedWithMkWeak :: (Eq k,Hashable k) => Int -> MkWeak -> ST s (h s k v)
    ...
    insertWeak   :: (Eq k, Hashable k) => h s k v -> k -> Weak v -> ST s ()
    insertWithMkWeak   :: (Eq k, Hashable k) => h s k v -> k -> v -> MkWeak -> ST s ()
    ...
    mapWeakM_    :: ((k,Weak v) -> ST s b) -> h s k v -> ST s ()
    ...
    finalize :: (Eq k,Hashable k) => h s k v -> ST s ()
