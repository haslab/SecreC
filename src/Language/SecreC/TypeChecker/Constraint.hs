...
equalsBase l (MSet b1) (MSet b2) = equalsComplex l b1 b2
equalsBase l (Set b1) (Set b2) = equalsComplex l b1 b2
...
coercesDimSizes l bvs e1@(loc -> ComplexT ct1@(CType s1 b1 d1)) x2@(loc -> ComplexT ct2@(CType s2 b2 d2)) = do
  ...
  where
    ...
    coercesDimSizes'' opts st d1 _ d2 _ | isJust (getNonWritableVar d1) || isJust (getNonWritableVar d2) = do
        tryInferCstr l opts (d1,d2) $ if doMulti
...
coercesSec' l bvs e1 ct1@(cSec -> Just s1) x2 s2 = do
  ...
  where
    ...
    coercesSec'' opts st s1 s2 | isJust (getNonWritableVar s1) || isJust (getNonWritableVar s2) = do
        tryInferCstr l opts (s1,s2) $ if doMulti
            then tcCstrM_ l $ Coerces (Just [SecT s1,SecT s2]) e1 x2
            else do
                -- s1 == s2
...
tryInferCstr :: (Vars GIdentifier (TcM m) x,ProverK loc m) => loc -> Options -> x -> TcM m () -> TcM m ()
tryInferCstr l opts x m = if (implicitContext opts == InferCtx)
    then do
        vs <- usedVs' x
        let inferrable = all (not . isWritable) vs
        if inferrable
            then do
...
comparesBase l isLattice = readable2 (comparesBase' isLattice) l
    ...
    comparesBase' isLattice t1@(MSet b1) t2@(MSet b2) = comparesComplex l isLattice b1 b2
    comparesBase' isLattice t1@(Set b1) t2@(Set b2) = comparesComplex l isLattice b1 b2
...
unifiesBase l = readable2 unifiesBase' l
    ...
    unifiesBase' (MSet b1) (MSet b2) = unifiesComplex l b1 b2
    unifiesBase' (Set b1) (Set b2) = unifiesComplex l b1 b2
...
resolveCVar l v isNotVoid = do
    ...
    case mb of
        Just t -> return t
        Nothing -> unresolvedVariable l v
