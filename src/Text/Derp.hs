...
data ParserRec p t a where
  ...
  Ter :: (Ord t)               => (t -> Bool) -> ParserRec p t t
...
terM tM = parser (Ter (`Set.member` tM)) (FPDecided False) (FPDecided False)

-- | Primitive version of `ter'.
terPrim :: (Ord t) => (t -> Bool) -> Parser t t
terPrim p = parser (Ter p) (FPDecided False) (FPDecided False)
...
deriveImp p' x' = deriveImpRec (parserRec p') x'
  where
    ...
    deriveImpRec (Ter p) (Token x t) | p x = eps x | otherwise = emp
